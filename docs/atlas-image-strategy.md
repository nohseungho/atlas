...
### 사진 중심 원칙 (2026-09-21 확정)

- **글은 사진과 실제 장면이 중심이다.** 텍스트 카드·표·체크리스트·인포그래픽 이미지는 보조 자료이며 **사진 수에 포함하지 않는다.**
- **글로벌(Blogger)**: 대표 이미지는 미지 마스터. 본문 장면 이미지는 본문 상황(서류 준비·공항·병원·여행 중 문제 해결 등)을 실제로 보여주는 사진에 미지 마스터를 그대로 합성해 외형을 일관되게 유지한다. 새 얼굴을 생성하지 않는다.
- **국내(Naver)**: 제품 실물·구성품·실사용 사진이 중심이고 수호 일러스트/카드는 보조다. 제품 사진은 파트너스 공식 소재 또는 권한이 확인된 사진만 쓴다(공식몰·상세페이지 이미지 무단 사용 금지).
- **사용 권한이 확인된 사진만 사용한다.** 퍼블릭 도메인·CC0·CC BY(귀속 표기)를 우선하고, CC BY-SA는 합성 이미지에 쓰지 않는다. 출처·작가·라이선스는 초안 `photoLicenses`(국내) / `visualAssets[].sceneSource`(글로벌)에 기록하고 본문에 귀속을 남긴다.
- **장수 채우기 금지**: 같은 이미지 반복, 단순 확대·크롭 변형, 내용과 무관한 장식 이미지로 이미지 수를 늘리지 않는다. 이미지마다 본문의 어느 문단·장면에 대응하는지 `placement`/`anchorKeywords`로 명시한다.
