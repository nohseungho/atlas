...
function globalArticle(overrides = {}) {
  return {
    ...
    visualAssets: keys.map((k, i) => ({
      ...
      channelId: ATLAS_CHANNEL_ID.GLOBAL_BLOGGER,
      characterId: "miji",
      faceMatch: { status: "pass", similarity: 0.72, threshold: 0.5, method: "arcface_buffalo_l", reference: "public/atlas/characters/ATLAS-MIJI-MASTER.png" },
    })),
    ...overrides,
  };
}
...
test("naver_monetized_candidate_only: missing affiliate link warns only after partners approval; pending passes", () => {
  const approved = coupangPartnersStatus({ COUPANG_PARTNERS_STATUS: "approved" });
  const pending = coupangPartnersStatus({});
  assert.equal(statusOf(evaluateKoreaDraft(koreaDraft({ affiliateUrl: "" }), { partners: approved }), "naver_monetized_candidate_only"), "warn");
  assert.equal(statusOf(evaluateKoreaDraft(koreaDraft({ affiliateUrl: "" }), { partners: pending }), "naver_monetized_candidate_only"), "pass");
  assert.equal(statusOf(evaluateKoreaDraft(koreaDraft(), { partners: approved }), "naver_monetized_candidate_only"), "pass");
});
...
test("global_character_face_match: missing, failed or below-threshold face evidence blocks", () => {
  assert.equal(statusOf(evalGlobal(globalArticle()), "global_character_face_match"), "pass");

  const none = globalArticle();
  delete none.visualAssets[0].faceMatch;
  assert.equal(statusOf(evalGlobal(none), "global_character_face_match"), "fail", "기록이 없으면 통과가 아니다");

  const operatorFail = globalArticle();
  operatorFail.visualAssets[3].faceMatch = { status: "fail", method: "operator_review", reason: "face differs from master" };
  const r = evalGlobal(operatorFail);
  assert.equal(statusOf(r, "global_character_face_match"), "fail");
  assert.ok(r.blocking.some((b) => b.code === "GLOBAL_CHARACTER_FACE_MISMATCH"));

  const low = globalArticle();
  low.visualAssets[1].faceMatch = { status: "pass", similarity: 0.31, threshold: 0.5 };
  assert.equal(statusOf(evalGlobal(low), "global_character_face_match"), "fail", "pass 표기라도 점수가 낮으면 막는다");
});

test("stored art_021 is blocked only by the re-publish block and missing face evidence", () => {
  ...
  const r = evalGlobal(article);
  // art_021은 얼굴 검수 게이트 이전에 공개된 글이라 검수 기록이 없다.
  assert.deepEqual(r.blocking.map((b) => b.code), ["ALREADY_PUBLISHED", "GLOBAL_CHARACTER_FACE_MISMATCH"], JSON.stringify(r.blocking));
  assert.equal(statusOf(r, "global_character_identity_review"), "warn");
});
