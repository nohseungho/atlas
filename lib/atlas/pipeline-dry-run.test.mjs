...
test("dry run: the real live corpus is untouched by this test", () => {
  ...
  // Not a fixed count: the pipeline is supposed to grow this corpus, so pinning
  // it to 8 made a successful publish look like a regression. What must hold is
  // that every published record is intact and that the dry run added nothing.
  assert.ok(published.length >= 8, `live posts should never drop below the 8 originals (got ${published.length})`);
  assert.ok(published.every((a) => /^https:\/\/atlas-money-2026\.blogspot\.com\//.test(a.publishedUrl)));
  assert.ok(published.every((a) => a.bloggerPostId), "every published post keeps its Blogger postId");
  assert.equal(new Set(published.map((a) => a.publishedUrl)).size, published.length, "no two posts share a public URL");
