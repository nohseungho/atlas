...
export function evaluateKoreaDraft(draft = {}) {
  const results = [];
  ...
  const monetized = Boolean(String(draft.affiliateUrl || "").trim());
  results.push(result("naver_monetized_candidate_only", monetized ? "pass" : "warn", monetized ? "" : "no affiliate link yet — publish gate will require one"));
