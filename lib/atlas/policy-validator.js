...
import { faceMatchIssues } from "./face-match.js";
...
export function evaluateGlobalArticle(article = {}, { html = "", succeededJobCount = 0 } = {}) {
  ...
  // 얼굴 일치 검수는 필수다. 기록이 없거나 fail이면 발행을 막는다.
  const faceIssues = faceMatchIssues(required);
  results.push(result("global_character_face_match", faceIssues.length ? "fail" : "pass", faceIssues.length ? faceIssues.join("; ") : `${required.length} matched`));
