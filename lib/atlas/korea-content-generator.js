...
export function generateKoreaProductArticle(input = {}) {
  ...
  const intro = `${name}은 화려한 기능보다 매일 자주 쓰는 기본기가 중요한 사람에게 먼저 볼 만한 제품입니다. 저는 제품을 추천할 때 장점만 나열하지 않고, 실제로 어떤 사람에게 맞고 어떤 사람에게는 굳이 필요하지 않은지까지 같이 보겠습니다.`;
...
결론
${name}은 모든 사람에게 무조건 최고라고 말할 제품이 아니라, 필요한 기능이 분명한 사람에게 가격과 사용 편의성을 보고 추천할 수 있는 제품입니다. 구매 전에는 현재 가격과 옵션, 배송 조건을 마지막으로 확인하는 것이 좋습니다.${affiliateUrl ? `\n\n제품 확인하기: ${affiliateUrl}` : ""}`;

  return {
    title: clean(input.title) || `${name}, 직접 써볼 만한 이유와 아쉬운 점`,
    bodyText,
...
export function generatePhilips3000KettleArticle(input = {}) {
  return generateKoreaProductArticle({
    ...
    title: input.title || "필립스 3000 시리즈 전기주전자, 매일 쓰기 좋은 이유와 아쉬운 점",
