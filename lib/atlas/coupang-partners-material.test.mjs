...
test("manual partners material is refused when it is not the expected product", () => {
  ...
  // 링크만 있으면 허용(이미지는 나중에), 링크도 이미지도 없으면 거부
  assert.equal(validatePartnersMaterial(target, { ...good, images: [] }).ok, true);
  assert.ok(validatePartnersMaterial(target, { ...good, link: "", images: [] }).issues.some((i) => /image/.test(i)));
});
