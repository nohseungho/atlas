...
import { isWellnessText } from "./wellness-scope.js";
import { isDisasterText } from "./disaster-scope.js";
...
// "before you book" is the travel-insurance reader's next step. A stretching
// routine has a different one, so the wellness category swaps the closing line —
// the rest of the description is still the article's own sentences.
const WELLNESS_CTA = "Read the full ATLAS routine.";
// A preparedness pin points at the checklist, never at a purchase and never at
// a donation page — the pin is a public-interest resource.
const DISASTER_CTA = "Read the full ATLAS preparedness guide.";
...
export function sanitizeOverlayText(value) {
  return String(value || "")
    .replace(/[,/\\]/g, " ")
    .replace(/[^\w\s&'’·.:-]/g, "")
    .replace(/\s+/g, " ")
...
export function buildPinDescription(article, { cta = CTA } = {}) {
  const pool = [
    article?.quickAnswer,
...
export function buildPinterestKit({ article, now = new Date() } = {}) {
  ...
  const topicText = `${article.category || ""} ${article.title || ""}`;
  const description = buildPinDescription(article, {
    cta: isDisasterText(topicText) ? DISASTER_CTA : isWellnessText(topicText) ? WELLNESS_CTA : CTA,
  });
