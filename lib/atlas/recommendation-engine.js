...
import {
  DISASTER_HAZARD_SIGNALS,
  DISASTER_ARTIFACT_SIGNALS,
  isDisasterText,
...
export function topicSignature(...parts) {
  ...
  return {
    ...
    // Disaster Preparedness cluster. Two preparedness guides differ by the
    // hazard they cover and the artifact they are about — a flood guide about
    // the building and one about the documents are not the same article.
    disaster: isDisasterText(text),
    hazard: matchIds(DISASTER_HAZARD_SIGNALS, text),
    artifact: matchIds(DISASTER_ARTIFACT_SIGNALS, text),
...
// Disaster-cluster duplication. Same rule shape as the wellness one: the same
// topic reworded, or the same hazard through the same artifact for the same
// reason. All three must agree, because "flood + documents" and "flood + the
// house" are two guides the launch seed set deliberately contains.
//
// Cross-category collisions are impossible: this returns null unless BOTH sides
// carry a disaster signal, and isDisasterText() refuses anything with an
// insurance signal in it.
function disasterCollision(a, b) {
  if (!a.disaster || !b.disaster) return null;
  if (jaccard(a.words, b.words) >= WELLNESS_TITLE_OVERLAP) return "동일 주제(제목 표현만 다름)";
  if (!overlaps(a.hazard, b.hazard)) return null;
  if (overlaps(a.artifact, b.artifact) && overlaps(a.intent, b.intent))
    return "동일 재해·동일 대비 품목 + 동일 검색 의도";
  return null;
}
...
export function signatureCollision(a, b) {
  ...
  const disaster = disasterCollision(a, b);
...
// Minimum slots held for each newer category while it has eligible seeds.
// Without a floor the dozen evergreen insurance seeds would fill every slot and
// a new category would never reach the screen; with one, the Money Hunter
// refill still keeps its priority over the static seeds.
const WELLNESS_RESERVED_SLOTS = 3;
const DISASTER_RESERVED_SLOTS = 3;
...
export function buildRecommendations({
  ...
  const axisOf = (c) => String(c.contentAxis?.id || "");
  const isWellnessCard = (c) => axisOf(c).startsWith("wellness-");
  const isDisasterCard = (c) => axisOf(c).startsWith("disaster-");
  const wellnessCards = passing.filter(isWellnessCard);
  const disasterCards = passing.filter(isDisasterCard);
  ...
  //   3. The newer categories take whatever the floor kept open, newest first
  //      so a just-added category is never the one that starves.
  //   4. Leftovers top the batch back up, so a short list in a new category
  //      never costs the batch a card.
  const travelCards = passing.filter((c) => !isWellnessCard(c) && !isDisasterCard(c));
  const reserved =
    Math.min(WELLNESS_RESERVED_SLOTS, wellnessCards.length) +
    Math.min(DISASTER_RESERVED_SLOTS, disasterCards.length);
  takeSeeds(travelCards, limit - reserved);
  takeRefill(limit);
  takeSeeds(disasterCards, limit - Math.min(WELLNESS_RESERVED_SLOTS, wellnessCards.length));
  takeSeeds(wellnessCards, limit);
  takeSeeds(disasterCards, limit);
