...
//   1. travel-insurance / travel-medical / trip-safety (the original scope),
//   2. Travel Wellness & Everyday Fitness (see wellness-scope.js), and
//   3. Disaster Preparedness & Emergency Essentials (see disaster-scope.js).
...
//
// The newer categories are MERGED in rather than written out again here: their
// axes, seeds and detectors live in their own scope modules, so the insurance
// rules below stay exactly as they were and the clusters cannot drift into each
// other's Hard Gate.
import {
...
// wellness and disaster clusters. The Hard Gate reads AXIS_IDS, so appending
// here is what makes a topic in a new category producible at all.
...
// The two newer detectors run FIRST because their signals are narrow and
// specific, while several travel signals below are broad enough to swallow
// them — "How to Stretch After a Long Flight" would be filed under flight
// disruption, and "Hurricane Preparedness Checklist" under trip preparation
// (SCOPE_SIGNALS matches "prepare" inside "preparedness").
//
// Neither detector can steal a travel topic. detectWellnessAxis returns null
// without a real movement signal, and detectDisasterAxis refuses any text
// carrying an insurance/coverage signal — which is what keeps the published
// "When to Buy Travel Insurance for Hurricane Season" on its insurance axis.
export function detectScope(text) {
  const t = String(text || "");
  if (OUT_OF_SCOPE.some((re) => re.test(t))) return { inScope: false, axis: null, outOfScope: true };
  const disaster = detectDisasterAxis(t);
  if (disaster) return { inScope: true, axis: disaster, outOfScope: false };
...
// All three clusters in one catalog. Order is irrelevant — recommendation-engine.js
// re-sorts by score — but keeping the travel seeds first preserves the existing
// tie-break order for the insurance cards.
export const EDITORIAL_SEEDS = [...TRAVEL_EDITORIAL_SEEDS, ...WELLNESS_SEEDS, ...DISASTER_SEEDS];
