...
import { checkWellnessSafety } from "./wellness-scope.js";
import { checkDisasterSafety } from "./disaster-scope.js";
...
export function runContentQa(article, { articles = [], affiliateApproved = false } = {}) {
  ...
  // 12b. Travel Wellness & Everyday Fitness safety gate.
  //      Category-scoped on purpose: it only runs for articles in that
  //      category, so every existing travel-insurance verdict is unchanged.
  //      A medical claim is a hard FAIL (ATLAS is not licensed to diagnose or
  //      treat); a missing "see a professional" line is a human-review flag,
  //      because the writer may have phrased it in a way no regex can prove.
  const wellness = checkWellnessSafety(article);
  if (wellness.applies) {
    add("wellnessClaims", "웰니스 — 진단·치료·보장 표현 없음",
      wellness.violations.length ? VERDICT.FAIL : VERDICT.PASS,
      wellness.violations.length
        ? `의료 단정 표현 ${wellness.violations.length}건 (${wellness.violations.join(", ")}) — 일반 웰니스 정보로 다시 쓰세요`
        : "진단·치료·보장·전후 비교 표현 없음");
    add("wellnessConsult", "웰니스 — 전문가 상담 권고 문구",
      wellness.hasConsultAdvice ? VERDICT.PASS : VERDICT.REVIEW,
      wellness.hasConsultAdvice
        ? "의사·물리치료사 상담 안내 포함"
        : "통증이 심하거나 지속될 때 전문가와 상담하라는 문장을 찾지 못했습니다 — 사람이 확인하세요.");
  }

  // 12c. Disaster Preparedness & Emergency Essentials safety gates.
  //      Five named checks, every one of them blocking, and every one of them
  //      category-scoped — they do not run for travel, insurance or wellness
  //      articles, so those verdicts are unchanged.
  //
  //      These exist because this is the only category where ATLAS writes about
  //      an ongoing disaster. The failure mode is not a factual error, it is
  //      looking like a business that monetizes other people's loss, so the
  //      structural separation between the relief half of the article and the
  //      product half is machine-checked rather than trusted.
  const disaster = checkDisasterSafety(article);
  const DISASTER_LABELS = {
    sensationalDisasterClaims: "재난 — 선정적·자극적 표현 없음",
    unverifiedCasualtyClaims: "재난 — 피해 수치 공식 출처 명시",
    donationAffiliateMixing: "재난 — 기부 영역·제휴 영역 분리",
    fakeDonationClaim: "재난 — 구매=기부 표현 없음",
    exploitativeCTA: "재난 — 피해를 이용한 구매 압박 없음",
  };
  for (const c of disaster.checks) {
    add(c.id, DISASTER_LABELS[c.id] || c.id, c.ok ? VERDICT.PASS : VERDICT.FAIL, c.reason);
  }
