...
import {
  ...
  resumeStateOf, progressOf, isLive, isTerminal, allowedNext, isRetryEligible,
...
test("only a QA-failed draft may be re-imported onto the same article", () => {
  // The pjob_005 case: content QA failed, so the job parked at REVIEW_REQUIRED
  // with the entry still at DRAFT_READY. A corrected package must re-run QA.
  assert.equal(
    isRetryEligible({ articleId: "art_011", jobStatus: "REVIEW_REQUIRED", state: STATE.DRAFT_READY }),
    true,
  );
});

test("retry is refused once QA passed or a human moved the article forward", () => {
  for (const state of [STATE.QA_PASSED, STATE.APPROVED, STATE.SCHEDULED, STATE.PUBLISHED]) {
    assert.equal(
      isRetryEligible({ articleId: "art_011", jobStatus: "REVIEW_REQUIRED", state }),
      false,
      `${state} must keep the idempotent return so an approved article is never rewritten`,
    );
  }
  // A job that is not parked at REVIEW_REQUIRED is not a retry either.
  assert.equal(isRetryEligible({ articleId: "art_011", jobStatus: "READY_TO_PUBLISH", state: STATE.DRAFT_READY }), false);
  // Nothing to retry onto without an existing article.
  assert.equal(isRetryEligible({ articleId: "", jobStatus: "REVIEW_REQUIRED", state: STATE.DRAFT_READY }), false);
  assert.equal(isRetryEligible(), false);
});
