...
// 링크·고지는 coupang-partners-status.js의 koreaLinkPlan이 정한다: 제휴 링크가 있을 때만 제휴 고지를 붙이고,
// 파트너스 승인 전에는 일반 상품 링크(제조사 공식 페이지 등) 또는 링크 없이 렌더한다.
...
export function bodyHtmlFromDraft(draft = {}) {
  ...
  const spacer = "<p><br></p>";
  const parts = [];
  const plan = koreaLinkPlan(draft);
  const linkLine = plan.url ? p(`<a href="${escapeHtml(plan.url)}" target="_blank">${escapeHtml(plan.label)}</a>`) : "";
  ...
  // 제휴 고지는 국내 확정 스타일에 따라 본문 최하단에 1회만 넣는다. 제휴 링크가 없으면 고지도 없다.
  if (plan.linkMode === "affiliate" && plan.disclosure) parts.push(spacer, p(`<span style="font-size:13px;color:#6b7280;">${escapeHtml(plan.disclosure)}</span>`));
  return parts.join("");
}
...
export function bodyPlainTextFromDraft(draft = {}) {
  const plan = koreaLinkPlan(draft);
  return [draft.bodyText || "", plan.url ? `제품 확인하기: ${plan.url}` : "", plan.linkMode === "affiliate" ? plan.disclosure : ""]
    .filter(Boolean)
    .join("\n\n");
}
