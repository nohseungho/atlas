...
import { ATLAS_CHANNEL_ID, validateChannelIdentity } from "./character-channel-policy";
...
export function runArticleQa(article, { articles = [], humanApproved = false } = {}) {
  ...
  const visualIssues = [];
  visualIssues.push(...validateChannelIdentity(article, ATLAS_CHANNEL_ID.GLOBAL_BLOGGER).issues);
  visuals.forEach((v, i) => {
