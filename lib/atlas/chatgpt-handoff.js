...
import {
  ...
  DISASTER_NARRATIVE_FLOW,
  DISASTER_SAFETY_RULES,
  DISASTER_PRODUCT_RULES,
  ...
  detectDisasterAxis,
  disasterAxis,
  suggestProductCategories as suggestDisasterProductCategories,
...
const NICHE_BY_BLOG = {
  blog_001:
    "overseas travel / travel insurance / travel health / safety + travel wellness & everyday fitness + disaster preparedness & emergency essentials",
...
// Disaster Preparedness & Emergency Essentials. The section order is not a
// suggestion here — it is the guarantee that relief information comes before
// preparedness and preparedness before any product, so the request file states
// it as a fixed structure plus the two forbidden zones.
function disasterProfile(candidate) {
  const axisId = String(candidate?.axis || "");
  const axis =
    disasterAxis(axisId) ||
    disasterAxis(detectDisasterAxis(`${candidate?.category || ""} ${candidate?.keyword || ""} ${candidate?.searchIntent || ""}`));
  if (!axis) return null;
  return {
    ...
    sectionOrderIsFixed: true,
    narrativeFlow: DISASTER_NARRATIVE_FLOW,
    masterSections: DISASTER_MASTER_SECTIONS,
    safetyRules: DISASTER_SAFETY_RULES,
    productRules: DISASTER_PRODUCT_RULES,
    productCategoryHints: suggestDisasterProductCategories(axis.id),
    suggestedTags: axis.tags,
    suggestedTopicEntities: axis.topicEntities,
    // 기부 영역과 제휴 영역은 문단이 아니라 구조로 갈라 둔다.
    donationPolicy: {
      disclosure: DONATION_DISCLOSURE,
      affiliateForbiddenSections: AFFILIATE_FORBIDDEN_SECTIONS,
      donationForbiddenSections: DONATION_FORBIDDEN_SECTIONS,
      verifiedReliefOrganizations: OFFICIAL_RELIEF_ORGS,
      matchingProgram: {
        enabled: donationMatchingActive(),
        reason: DONATION_MATCHING.reason,
        rule: "A purchase must never be described as donating, funding relief, or being matched. No exceptions while enabled is false.",
      },
      noVerifiedLinkFallback:
        "If no verified relief organisation has an active appeal for this event, say so and link the official information source. Never substitute a shop link, a crowdfunding page, or an affiliate link for a donation link.",
    },
    pinDirections: DISASTER_PIN_TEMPLATES.map((t) => ({ id: t.id, label: t.label, imageRole: t.imageRole, tone: t.tone })),
    pinBoardSuggestions: axis.pinBoards,
    pinRules: DISASTER_PIN_RULES,
    imageDirection:
      "Informational and calm. Illustrate the checklist, the kit, the bag, or the plan — flat-lay supplies, a labelled diagram, " +
      "a family filling in a plan. NEVER an image of victims, casualties, bodies, injury, or the destruction of a real community, " +
      "and never a dramatised disaster scene used to pull a click.",
  };
}
...
export function resolveCategoryProfile(candidate) {
  const disaster = disasterProfile(candidate);
    ...
    note:
      ...
      (categoryProfile
        ? ` Category: ${categoryProfile.category} (${categoryProfile.axisLabel}). Follow contentCategory.narrativeFlow and contentCategory.masterSections, not the travel-insurance skeleton. ` +
          (categoryProfile.category === DISASTER_CATEGORY_LABEL
            ? "The section order is FIXED and the two zones are absolute: sections 1-4 carry no product, no affiliate link and no purchase CTA; section 6 carries no donation link. " +
              `State "${DONATION_DISCLOSURE}" in the How to Help section. Never write that a purchase donates, funds relief, or is matched — ATLAS has no such program. ` +
              "Attribute every casualty or damage figure to a named official source and its date."
