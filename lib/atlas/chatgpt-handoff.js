...
import { buildLettersBlock, castForWeek } from "./letters-cast.js";
...
// §4 — the ATLAS article skeleton. Every guide carries these sections so a
// reader always gets the cost answer and at least one artifact (table /
// decision matrix / checklist) they can act on without reading the prose twice.
...
export function buildHandoffRequest({ jobId, blog, candidate, existingArticles = [], unusedCandidates = [], persona, template, cast }) {
  ...
  return {
    ...
    note:
      ...
      `ATLAS Letters: the featured image shows ${letters?.heroCharacterId || ""}, generated from ${letters?.masterFileName || ""} with the face locked (see letters.faceLock). ` +
