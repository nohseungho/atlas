...
async function setTitle(scope, title) {
  ...
  await titleEl.click();
  await titleEl.press(process.platform === "darwin" ? "Meta+A" : "Control+A");
  await titleEl.fill ? await titleEl.fill(title).catch(async () => { await titleEl.press("Backspace"); await titleEl.type(title); }) : await titleEl.type(title);
}
...
async function setBody(scope, draft) {
  ...
  if (draft.contentType === "existing_post_update" && draft.updateMode === "images_only") {
    return bodyEl;
  }
  ...
  const text = [
    ...
    draft.bodyText || "",
    draft.affiliateUrl ? `\n제품 확인하기: ${draft.affiliateUrl}` : "",
  ].filter(Boolean).join("\n\n");

  await bodyEl.click();
  await bodyEl.press(process.platform === "darwin" ? "Meta+A" : "Control+A");
  await bodyEl.press("Backspace");
  await bodyEl.type(text, { delay: 1 });
  return bodyEl;
}
  ...
  try {
    ...
    if (!publish) {
      return {
        ...
        editorUrl: page.url(),
        imageUpload: images,
        message: "네이버 편집기에 자동 반영했습니다. 발행은 승인 전이라 실행하지 않았습니다.",
      };
    }
    ...
    return {
      ...
      imageUpload: images,
      message: "네이버 발행 동작을 완료했습니다.",
    };
  } catch (error) {
    if (error?.code === "NAVER_LOGIN_REQUIRED") {
      // Keep the window open so the user can complete the one-time login.
