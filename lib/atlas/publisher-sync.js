  ...
  if (result?.status === "matched") {
    ...
    return {
      ...
      updates: {
        ...
        bloggerStatus: "LIVE",
        bloggerSyncedAt: now,
        // Blogger's own URL is authoritative: a stored URL goes stale whenever
        // Blogger re-slugs a post (duplicate title ⇒ "...-for_02069363082.html"),
        // leaving ATLAS pointing at a 404. Stored value is kept only as fallback.
        publishedUrl: post.url || article.publishedUrl || "",
        publishedAt: article.publishedAt || post.published || now,
