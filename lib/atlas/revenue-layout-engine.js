...
// Placement sentinel for assets that the body HTML already renders itself
// (CHATGPT_HANDOFF packages ship one complete MASTER HTML with its figures
// inlined). Such assets stay in visualAssets as the image manifest, but the
// chrome assembler must never inject a second copy of them.
export const EMBEDDED_PLACEMENT = "embedded";
...
//
// Two assets sharing one placement is a data defect (art_010 shipped
// featured+action on afterByline and comparison+checklist on afterFaq). The
// old last-wins assignment silently dropped the earlier, higher-priority slot,
// so which image surfaced depended on array order. First-wins makes that
// deterministic; the losing asset is simply not injected rather than
// overwriting a slot it does not own.
export function resolveVisualAssetsByPlacement(article, mode) {
  ...
  for (const asset of assets) {
    if (asset?.placement === EMBEDDED_PLACEMENT) continue;
    const resolved = resolveVisualAsset(asset, mode);
    if (!resolved || map[resolved.placement]) continue;
...
// Body markdown (Official Sources lists especially) often carries a raw
// https:// string. Readers must be able to click it, and the link must not
// leak the referrer or let the target script the opener. Input is already
// HTML-escaped text; anchors that exist are left untouched (the regex never
// matches inside href="..." because the quote breaks the URL character class).
const BARE_URL_PATTERN = /(^|[^"'=>\w])(https?:\/\/[^\s<>"']+?)([.,;:)\]]*)(?=$|[\s<])/g;

export function autolinkUrls(escapedText) {
  const text = String(escapedText || "");
  if (!/https?:\/\//i.test(text)) return text;
  return text.replace(BARE_URL_PATTERN, (m, lead, url, trail) =>
    `${lead}<a href="${url}" target="_blank" rel="noopener noreferrer">${url}</a>${trail}`
  );
}

// True when the HTML still shows an https:// string outside an <a> element —
// the failure mode the autolink above exists to prevent.
export function hasBareUrlText(html) {
...
// ─── Assembly integrity: what the body HTML already carries ──────────────────
// The chrome assembler (buildRevenueHtml) adds FAQ / Sources / Quick Answer /
// disclaimer / figure sections from the article's structured fields. When the
// body HTML already contains those same sections — which is always true for a
// CHATGPT_HANDOFF MASTER package — adding them again produced a post with two
// FAQ sections, two Sources lists, and more images than the article owns
// (art_010: 8 <img> for 5 assets). These helpers let the assembler ask the body
// what it already provides and skip exactly those sections. The rule is
// article-agnostic: it keys off the body's own content, never an article id.

// The same Cloudinary asset can appear with different transform params or
// cache-busting query strings. Compare on hostname + public_id path only, so a
// re-transformed variant of an embedded image is still recognized as embedded.
export function normalizeImageSrc(value) {
  ...
  const publicPath = parsed.pathname.replace(/\/image\/upload\/[^/]*\/(?:v\d+\/)?/i, "/image/upload/");
  return `${parsed.hostname}${publicPath}`.toLowerCase();
}

const FAQ_HEADING_PATTERN =
  /<h[23][^>]*>[^<]*(?:\bfaq\b|frequently asked questions|자주\s*묻는\s*질문)[^<]*<\/h[23]>/i;
const SOURCES_HEADING_PATTERN = /<h[23][^>]*>[^<]*(?:\bsources\b|\breferences\b|출처)[^<]*<\/h[23]>/i;
const QUICK_ANSWER_HEADING_PATTERN = /<h[23][^>]*>[^<]*(?:quick answer|한\s*줄\s*요약|핵심\s*답변)[^<]*<\/h[23]>/i;
const DISCLAIMER_PATTERN = /\bdisclaimer\b|\bdisclosure\b|안내\s*사항|주의\s*사항/i;

export function inspectBodyHtml(bodyHtml) {
  const s = String(bodyHtml || "");
  const imageSrcs = new Set(
    Array.from(s.matchAll(/<img[^>]+src=["']([^"']+)["']/gi), (m) => normalizeImageSrc(m[1])).filter(Boolean)
  );
  return {
    hasFaq: FAQ_HEADING_PATTERN.test(s),
    hasSources: SOURCES_HEADING_PATTERN.test(s),
    hasQuickAnswer: QUICK_ANSWER_HEADING_PATTERN.test(s),
    hasDisclaimer: DISCLAIMER_PATTERN.test(s),
    imageCount: imageSrcs.size,
    hasImage: (src) => imageSrcs.has(normalizeImageSrc(src)),
  };
}
