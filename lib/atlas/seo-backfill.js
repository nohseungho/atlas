...
// The MASTER itself ships a bare "Related ATLAS Guides" H2 with an empty <ul>:
// the request file tells ChatGPT to leave exactly that and let ATLAS fill it
// (chatgpt-handoff.js). ATLAS fills it by appending its own atlas-related
// section, so unless the skeleton is stripped too the finished post carries the
// same H2 twice. Matched only after the real section is removed above —
// otherwise this would gut the generated block, which has that H2 inside it.
const RELATED_SKELETON_RE = /<h2[^>]*>\s*Related ATLAS Guides\s*<\/h2>\s*(?:<ul[^>]*>[\s\S]*?<\/ul>)?\s*/gi;

// Strips any block a previous backfill appended plus the MASTER's empty
// skeleton, so re-running never stacks a second "Related ATLAS Guides" onto the
// post and the generated block is always the only one.
export function stripRelatedBlock(html) {
  return String(html || "")
    .replace(RELATED_BLOCK_RE, "")
    .replace(RELATED_SKELETON_RE, "")
    .replace(/\s+$/, "");
}
