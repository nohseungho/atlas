...
test("stored art_021 assembles to 5 placed images / 1 FAQ / 1 Sources / 1 disclaimer and no affiliate surface", () => {
  ...
  // Stand in for the Cloudinary URLs the prepare step will write, so publish mode renders.
  const prepared = { ...article, visualAssets: article.visualAssets.map((a) => ({ ...a, publicUrl: url(a.key) })) };
  const { html } = buildRevenueHtml(prepared, minimalMarkdownToHtml(prepared.bodyMarkdown), { mode: "publish" });
  ...
  // Travel articles have no afterByline slot, so featured follows Quick Answer; the rest land in their own section.
  const order = srcs.map((s) => s.replace(/\?.*$/, "").split("/").pop());
