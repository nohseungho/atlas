...
function buildVisualAssetFigure(resolvedAsset, { eager } = {}) {
  ...
  return `<figure style="margin:28px 0 32px;font-family:system-ui,-apple-system,sans-serif;">
  <img src="${esc(resolvedAsset.src)}" alt="${esc(resolvedAsset.alt)}" width="${esc(resolvedAsset.width)}" height="${esc(resolvedAsset.height)}" ${loadingAttrs} style="display:block;width:100%;max-width:100%;height:auto;aspect-ratio:16/9;object-fit:${resolvedAsset.fit};border-radius:12px;border:1px solid #e5e7eb;box-shadow:0 1px 3px rgba(15,23,42,.08);" />
</figure>`;
}
