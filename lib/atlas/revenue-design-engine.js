...
function buildVisualAssetFigure(resolvedAsset, { eager } = {}) {
  ...
  // Breathing room above and below plus a hairline frame keep an in-body
  // figure from reading as a pasted-in block, especially at phone width.
  return `<figure style="margin:${GLOBAL_LAYOUT.figureMargin};font-family:system-ui,-apple-system,sans-serif;">
  <img src="${esc(resolvedAsset.src)}" alt="${esc(resolvedAsset.alt)}" width="${esc(resolvedAsset.width)}" height="${esc(resolvedAsset.height)}" ${loadingAttrs} style="display:block;width:100%;max-width:100%;height:auto;aspect-ratio:16/9;object-fit:${resolvedAsset.fit};border-radius:${GLOBAL_LAYOUT.figureRadiusPx}px;border:1px solid #e5e7eb;box-shadow:0 1px 3px rgba(15,23,42,.08);" />
</figure>`;
}
...
function injectSectionVisuals(bodyHtml, article, visualsByPlacement, body) {
  if (!bodyHtml) return bodyHtml;
  const headings = extractH2s(article.bodyMarkdown);
  // The hero box already shows article.heroImage at the top; the same asset
  // must not appear a second time as an in-body figure.
  // Local preview resolves the same asset to its localSrc, so both spellings
  // of a hero-equivalent visualAssets entry count as "already shown".
  const heroSrc = normalizeImageSrc(resolveHeroImage(article)?.url);
  const heroSrcs = new Set();
  if (heroSrc) {
    heroSrcs.add(heroSrc);
    for (const a of Array.isArray(article.visualAssets) ? article.visualAssets : []) {
      if (normalizeImageSrc(a?.publicUrl) === heroSrc && a?.localSrc) heroSrcs.add(normalizeImageSrc(a.localSrc));
    }
  }
  let out = bodyHtml;
  for (const [placement, asset] of Object.entries(visualsByPlacement)) {
    if (!placement.startsWith(SECTION_PLACEMENT_PREFIX)) continue;
    if (body.hasImage(asset.src)) continue; // already rendered in the body — no second copy
    if (heroSrcs.has(normalizeImageSrc(asset.src))) continue; // shown as the hero — no second copy
...
// Splits the assembled body at its second H2 when the first section is the
// body's own Quick Answer. Relies on the atlas-h2-N ids markdownToHtml assigns,
// so hand-off bodies without those ids (or without a Quick Answer) are left whole.
const QUICK_ANSWER_H2 = /<h2 id="atlas-h2-0"[^>]*>[^<]*(?:quick answer|한\s*줄\s*요약|핵심\s*답변)[^<]*<\/h2>/i;
function splitLeadSection(bodyHtml, hasQuickAnswer) {
  const html = bodyHtml || "";
  if (!hasQuickAnswer || !QUICK_ANSWER_H2.test(html)) return { lead: "", rest: html };
  const cut = html.indexOf('<h2 id="atlas-h2-1"');
  if (cut === -1) return { lead: "", rest: html };
  return { lead: html.slice(0, cut).trim(), rest: html.slice(cut) };
}
...
// reviewedAt/independenceNote) when present. Falls back to the old generic one-line credit for
...
function buildTrustBox(article, chrome) {
  ...
  if (trust && (trust.authorName || trust.role || trust.methodology || trust.reviewedAt)) {
    ...
    // An article may supply its own independence note (e.g. an official-source
    // information guide with no affiliate surface); otherwise the chrome default.
    const independenceNote = (trust.independenceNote || "").trim() || chrome.trustIndependenceNote;
    lines.push(`  <p style="margin:0;font-size:11px;color:#9ca3af;line-height:1.5;">${esc(independenceNote)}</p>`);
...
export function buildRevenueHtml(article, bodyHtml, { mode = "publish" } = {}) {
  ...
  // Assembly integrity: never add a section the body HTML already carries.
  // A CHATGPT_HANDOFF MASTER body ships its own figures, FAQ, Sources, and
  // disclaimer; re-adding them from the article fields is what produced the
  // duplicated post structure. Articles whose body is plain prose (the classic
  // markdown pipeline) match none of these and get the full chrome as before.
  const body = inspectBodyHtml(bodyHtml);
  const visualsByPlacement = resolveVisualAssetsByPlacement(article, mode);
  const slotFigure = (asset, opts) => (asset && body.hasImage(asset.src) ? "" : buildVisualAssetFigure(asset, opts));
  const leadEditorialImage = slotFigure(visualsByPlacement.afterByline, { eager: true });
  const comparisonInfographicImage = slotFigure(visualsByPlacement.afterComparisonCriteria);
  const beforeBuyInfographicImage = slotFigure(visualsByPlacement.afterFaq);
  ...
  const bodyWithVisuals = injectSectionVisuals(bodyHtml, article, visualsByPlacement, body);
  // Snippet quality: Blogger derives og:description and the home-page excerpt
  // from the first text in the post. When the body opens with its own Quick
  // Answer section, that section is hoisted above the TOC/takeaways chrome so
  // the excerpt reads as the answer instead of "Last updated … Table of Contents …".
  const { lead: leadSection, rest: bodyRest } = splitLeadSection(bodyWithVisuals, body.hasQuickAnswer);
  ...
  const quickAnswerBox = body.hasQuickAnswer ? "" : buildQuickAnswerBox(article);
  const comparisonCriteriaBox = buildComparisonCriteriaBox(article, chrome);
  // The affiliate disclosure is a compliance surface tied to a live affiliate
  // link, so it is never suppressed — only the general editorial disclaimer is.
  const editorialDisclosureBox = body.hasDisclaimer ? "" : buildEditorialDisclosureBox(article, chrome);
  ...
  const faqBlock = body.hasFaq ? "" : buildFaqBlock(article, chrome);
  const relatedBox = buildRelatedArticlesBox(article, chrome);
  const trustBox = buildTrustBox(article, chrome);
  const sourcesBox = body.hasSources ? "" : buildSourcesBox(article, chrome);
  ...
  // Reading-width container (operating policy "global_layout_defaults"):
  // caps the desktop line length without touching the mobile flow. The
  // data-attribute lets the policy validator confirm the wrapper is present.
  const html = `<div data-${GLOBAL_LAYOUT.layoutMarker} style="max-width:${GLOBAL_LAYOUT.maxWidthPx}px;margin:0 auto;">\n${parts.filter(Boolean).join("\n\n")}\n</div>`;
