  ...
  return stampChannelIdentity({
    ...
    images: Array.isArray(input.images)
      ? input.images.map((img, index) => ({
          ...
          // 장면 생성 지시와 프롬프트는 편집을 거쳐도 유지한다. 이게 사라지면
          // 이미지 생성 요청서의 prompt가 비어 버린다.
          ...(img?.prompt ? { prompt: String(img.prompt) } : {}),
          ...(img?.direction && typeof img.direction === "object" ? { direction: img.direction } : {}),
          ...(img?.sceneArtSource ? { sceneArtSource: String(img.sceneArtSource) } : {}),
