...
test("reconcile: a re-slugged Blogger post refreshes the stored publishedUrl", () => {
  const articles = [
    {
      id: "art_005",
      title: "How to Build a Travel Health Kit",
      status: "published",
      bloggerPostId: "1303068607823477518",
      publishedUrl: "https://b.com/2026/07/how-to-build-travel-health-kit-for.html",
    },
  ];
  const livePosts = [
    post(
      "1303068607823477518",
      "How to Build a Travel Health Kit",
      "https://b.com/2026/07/how-to-build-travel-health-kit-for_02069363082.html",
    ),
  ];
  const { rows } = reconcileArticles({ articles, livePosts });

  assert.equal(rows[0].linkStatus, "linked");
  assert.equal(
    rows[0].updates.publishedUrl,
    "https://b.com/2026/07/how-to-build-travel-health-kit-for_02069363082.html",
    "the LIVE Blogger URL wins over the stale stored one",
  );

  applyRowUpdates(articles[0], rows[0]);
  assert.equal(articles[0].publishedUrl, rows[0].updates.publishedUrl);
});
