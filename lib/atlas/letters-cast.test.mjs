...
test("next week 2026-08-17: the roles swap in text — 수호 asks, 미지 answers — but the hero stays 미지", () => {
  ...
  assert.equal(cast.heroCharacterId, "miji");
  assert.equal(cast.masterFileName, "ATLAS-MIJI-MASTER.png");
  assert.equal(cast.heroLock, "global_blogger:miji");
...
test("the hero is 미지 in every week, whoever asks (global channel lock)", () => {
  for (const d of ["2026-08-10", "2026-08-17", "2026-08-24", "2026-11-30"]) {
    const cast = castForWeek(`${d}T12:00:00+09:00`);
    assert.equal(cast.heroCharacterId, "miji");
    assert.equal(cast.masterFileName, CHARACTERS.miji.masterFileName);
    assert.equal(cast.masterAssetPath, "public/atlas/characters/ATLAS-MIJI-MASTER.png");
  }
});
...
const MASTER_SHA256 = {
  ...
  // 2026-09-20: 실사 사진(458454af…)은 확정 수호가 아니어서 archive/로 옮기고, 멀티탭 원본
  // 일러스트(public/atlas/korea/suho/multitap/featured.png)를 잘라낸 파일로 교체했다.
  suo: "a6ff9948f475de4a7eba22b6fabd29ad8ae34d0b71acdd25238c7315dc2694ac",
};

// 수호 마스터는 확정 원본 일러스트의 바이트 단위 crop이어야 한다(재생성 금지).
test("수호 마스터는 확정 멀티탭 일러스트의 crop이다", async () => {
  const sharp = (await import("sharp")).default;
  const source = path.join(repoRoot, "public/atlas/korea/suho/multitap/featured.png");
  const master = await sharp(path.join(repoRoot, CHARACTERS.suo.masterAssetPath)).raw().toBuffer({ resolveWithObject: true });
  const crop = await sharp(source).extract({ left: 268, top: 305, width: 385, height: 455 }).raw().toBuffer({ resolveWithObject: true });
  assert.equal(master.info.width, 385);
  assert.equal(master.info.height, 455);
  assert.ok(master.data.equals(crop.data), "master pixels differ from the confirmed illustration crop");
});
