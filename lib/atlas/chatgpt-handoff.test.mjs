...
import {
  automationMode, buildHandoffRequest, buildRecommendationCandidate, validateImageBase64,
...
function validPackage() {
  return {
    ...
    // 150-160 chars — the SEO standard the importer now enforces.
    metaDescription: "A step-by-step guide to filing a travel insurance claim after a medical emergency abroad, including the documents, deadlines, and mistakes that cost money.",
...
// ─── R2 recommendation card → 수동 제작요청 ──────────────────────────────────
const R2_CANDIDATE = {
  priority: 2,
  ...
  contentAxis: { id: "traveler-segment-coverage", label: "Family, Senior, Student and Frequent Traveler Coverage" },
  score: 80,
};

test("buildRecommendationCandidate: topic-derived id is stable, never a fake moneyScore", () => {
  const a = buildRecommendationCandidate(R2_CANDIDATE);
  const b = buildRecommendationCandidate({ ...R2_CANDIDATE, score: 41 });
  assert.equal(a.id, "rec_student-travel-insurance-for-studying-abroad-a-practical-guide");
  assert.equal(a.id, b.id); // same topic → same id → same job → no duplicate
  assert.equal(a.keyword, R2_CANDIDATE.title);
  assert.equal(a.searchIntent, R2_CANDIDATE.searchIntent);
  assert.equal(a.category, R2_CANDIDATE.contentAxis.label);
  assert.equal(a.moneyScore, null);
});

test("buildRecommendationCandidate: a topic-less card yields no request", () => {
  assert.equal(buildRecommendationCandidate(null), null);
  assert.equal(buildRecommendationCandidate({ title: "   " }), null);
  assert.equal(buildRecommendationCandidate({ title: "한국어 주제" }), null);
});

test("R2 request file carries topic, keyword, jobId and the package requirements", () => {
  const req = buildHandoffRequest({
    jobId: "pjob_007",
    blog: { id: "blog_001", name: "ATLAS Money Blog 01" },
    candidate: buildRecommendationCandidate(R2_CANDIDATE),
    existingArticles: [{ id: "art_003", title: "T", slug: "t", searchIntent: "informational", status: "published" }],
    unusedCandidates: [{ id: "kw_020", keyword: "adventure travel insurance exclusions", category: "travel insurance" }],
  });
  assert.equal(req.jobId, "pjob_007");
  assert.equal(req.selectedCandidate.keyword, R2_CANDIDATE.title);
  assert.equal(req.selectedCandidate.searchIntent, R2_CANDIDATE.searchIntent);
  assert.equal(req.selectedCandidate.moneyHunterId, "rec_student-travel-insurance-for-studying-abroad-a-practical-guide");
  assert.equal(req.packageSchemaExpected, PACKAGE_SCHEMA);
  assert.equal(req.masterSections.length, 8);
  assert.equal(req.qaCriteria.imageCount, 5);
  assert.equal(req.unusedCandidates[0].moneyHunterId, "kw_020");
...
// ─── 상품 연결 (Product Center → 제작 요청 파일) ────────────────────────────

const LINKED_PRODUCT = {
  productId: "product_1565379b-3375-4772-a180-2496c1251ec9",
  ...
  affiliateStatus: "AFFILIATE_PENDING",
  affiliateStatusLabel: "제휴 링크 대기",
  affiliateLink: "",
  currency: "USD",
  currentPrice: 36.95,
  priceSource: "nitecorestore.com 상품 페이지",
  priceCheckedAt: "2026-08-19T01:00:00.000Z",
  benefit: "야간 하산에서 두 손을 쓰게 해 준다",
  features: ["Max output: 400 lumens"],
  imageRefs: [{ imageId: "img_1", name: "nu25.jpg" }],
};

test("제작 요청 파일에 linkedProductIds와 상품 스냅샷이 들어간다", () => {
  const req = buildHandoffRequest({
    jobId: "pjob_014",
    blog: { id: "blog_001", name: "ATLAS Money Blog 01" },
    candidate: { id: "kw_x", keyword: "night hiking safety" },
    linkedProducts: [LINKED_PRODUCT],
  });
  assert.deepEqual(req.linkedProductIds, [LINKED_PRODUCT.productId]);
  const snap = req.linkedProducts.products[0];
  assert.equal(snap.name, LINKED_PRODUCT.name);
  assert.equal(snap.category, "Outdoor Safety");
  assert.equal(snap.productUrl, LINKED_PRODUCT.productUrl);
  assert.equal(snap.currentPrice, 36.95);
  assert.equal(snap.currency, "USD");
  assert.equal(snap.priceCheckedAt, LINKED_PRODUCT.priceCheckedAt);
  assert.equal(snap.benefit, LINKED_PRODUCT.benefit);
  assert.equal(snap.imageRefs[0].imageId, "img_1");
});

test("제휴 링크가 없으면 대기 상태가 실리고 가짜 링크·구매 버튼이 금지된다", () => {
  const req = buildHandoffRequest({
    jobId: "pjob_014",
    blog: { id: "blog_001" },
    candidate: null,
  ...
  assert.deepEqual(req.linkedProducts.affiliatePendingIds, [LINKED_PRODUCT.productId]);
  assert.equal(req.linkedProducts.products[0].affiliateLink, "");
  assert.ok(req.linkedProducts.editorialRules.some((r) => /do NOT invent an affiliate link/i.test(r)));
  assert.match(req.note, /never invent an affiliate link, price, or buy button/i);
});

test("연결된 상품이 없으면 요청 파일에 빈 상품 블록이 생기지 않는다", () => {
  const req = buildHandoffRequest({ jobId: "pjob_015", blog: { id: "blog_001" }, candidate: null });
  assert.deepEqual(req.linkedProductIds, []);
  assert.equal(req.linkedProducts, null);
});
