...
export const CHANNEL_PROFILE = Object.freeze({
  ...
  global: Object.freeze({
    ...
    width: 1344,
    height: 768,
    // 공통 얼굴 레퍼런스(2026-09-24): 마스터 + 승인된 art_021 세트 5장을 같은 크기 얼굴 크롭으로 만든 것.
    // 한 장에만 맞추면 얼굴을 붙인 듯한 느낌이 나므로 여러 장의 평균 얼굴로 고정한다.
    extraReferences: [1, 2, 3, 4, 5, 6].map((i) => `public/atlas/characters/references/miji/miji-ref-${i}.png`),
    ...
    faceId: { preset: "FACEID PLUS V2", loraStrength: 0.6, weight: 0.9, weightV2: 1.2, startAt: 0.1, endAt: 1, useExtraReferencesOnly: true },
    // 저장된 요청 프롬프트에는 옛 연출 문구가 섞여 있을 수 있어 장면 의도(첫 문장)만 쓰고
    // 연출은 현재 scene-direction 값으로 채운다.
    sceneFromIntentOnly: true,
    subject: "a Korean woman in her early 30s, dark-brown hair color, soft rounded oval face, gentle smiling eyes, warm brown almond-shaped eyes, small nose, soft jawline, youthful bright complexion, kind warm smile",
    ...
    negative: "(two people:1.4), (second person:1.3), twins, duplicate person, (pasted face:1.3), face swap artifact, mismatched face lighting, uncanny face, distorted face, heavy airbrushed skin, (older woman:1.3), wrinkles, gaunt face, sharp cheekbones, anime, illustration, cartoon, painting, 3d render, man, child, text, letters, watermark, logo, readable signage, deformed hands, extra fingers, lowres, blurry, collage, split screen, frame, border, studio portrait",
    ipWeight: 0.75,
    ipStartAt: 0.2,
  }),
});
...
export function generationPrompt(channel, requestPrompt, direction = null) {
  ...
  const lead = [
    ...
    direction?.makeup && `(${direction.makeup}:1.1)`,
...
function faceIdWorkflow(wf, profile, { seed, referenceNames, prefix }) {
  ...
  delete wf.ipa;
  delete wf.clipv;
  // 공통 레퍼런스(같은 크기 얼굴 크롭)를 한 배치로 묶는다. 첫 번째(원본 마스터)는 크기가 달라 제외한다.
  const names = f.useExtraReferencesOnly && referenceNames.length > 1 ? referenceNames.slice(1) : referenceNames.slice(0, 1);
  names.forEach((name, i) => {
    wf[`ref${i}`] = { class_type: "LoadImage", inputs: { image: name } };
  });
  let faceBatch = ["ref0", 0];
  for (let i = 1; i < names.length; i += 1) {
    wf[`fbatch${i}`] = { class_type: "ImageBatch", inputs: { image1: faceBatch, image2: [`ref${i}`, 0] } };
    faceBatch = [`fbatch${i}`, 0];
  }
  wf.faceid_loader = {
