...
const SUHO_STYLE = "warm anime-style illustration of a Korean home interior, soft daylight, clean lines";
...
export function koreaInfoImages(topic) {
  ...
  const intents = topic.sceneIntents || {};
  const slots = [
    {
      id: "img_info_why",
      role: "info_why",
  ...
  return slots.map((slot) => ({
    ...
    direction: directionFor("korea", slot.role),
    prompt: scenePrompt({
      identityPrompt: ATLAS_CHARACTERS.suho.identityPrompt,
      sceneIntent: intents[slot.role],
      direction: directionFor("korea", slot.role),
      style: SUHO_STYLE,
    }),
  }));
