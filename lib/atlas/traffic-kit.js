...
import {
  ...
  WELLNESS_PIN_OFFSET_DAYS,
  detectWellnessAxis,
  suggestPinBoards,
} from "./wellness-scope.js";
import {
  DISASTER_PIN_TEMPLATES,
  DISASTER_PIN_OFFSET_DAYS,
  DISASTER_CATEGORY_LABEL,
  detectDisasterAxis,
  suggestPinBoards as suggestDisasterPinBoards,
...
// Travel Wellness & Everyday Fitness pins. "Common mistakes before you book" is
// the wrong second angle for a stretching routine, and the category's actual
// traffic comes from a fourth kind of pin the insurance set has no use for:
// the photo-real 미지 lifestyle shot. So this category gets its own four.
export const WELLNESS_VARIANTS = WELLNESS_PIN_TEMPLATES.map(({ id, label }) => ({ id, label }));
...
const WELLNESS_CTA = Object.fromEntries(WELLNESS_PIN_TEMPLATES.map((t) => [t.id, t.cta]));

// Disaster Preparedness & Emergency Essentials pins. Five informational
// directions only: a checklist, a preparedness card, a go-bag, a family plan,
// and a before/during/after card. There is deliberately no "shocking damage"
// direction — a pin in this category never shows victims or destruction, and
// never links anywhere but the ATLAS preparedness guide.
export const DISASTER_VARIANTS = DISASTER_PIN_TEMPLATES.map(({ id, label }) => ({ id, label }));

const DISASTER_CTA = Object.fromEntries(DISASTER_PIN_TEMPLATES.map((t) => [t.id, t.cta]));
...
const TRAVEL_ASSET_ORDER = [
  ["featured", "context"],
  ["comparison", "context", "action"],
  ["checklist", "action", "context"],
];

// The wellness set needs a fourth image, and the 미지 lifestyle pin wants the
// "action" shot (the person actually doing the move) rather than a diagram.
const WELLNESS_ASSET_ORDER = [
  ["featured", "context"],
  ["context", "comparison", "checklist"],
  ["checklist", "comparison", "context"],
  ["action", "featured", "context"],
];

// Five pins, five images — the article's whole manifest, one per direction.
const DISASTER_ASSET_ORDER = [
  ["checklist", "comparison", "context"],
  ["comparison", "checklist", "context"],
  ["context", "action", "featured"],
  ["action", "context", "featured"],
  ["featured", "context", "comparison"],
];

// Different image per variant, from the article's own five uploads. Falls back
// to any unused asset, and only repeats one when the article has fewer distinct
// images than the variant set needs.
function pickAssets(article, order = TRAVEL_ASSET_ORDER) {
...
// Whether this article belongs to Travel Wellness & Everyday Fitness. Read from
// the article's own category/axis first and only then from its title, so a
// travel-insurance guide can never be re-routed onto the wellness pin set.
// Whether this article belongs to Disaster Preparedness & Emergency Essentials.
// Checked before the wellness test, and the underlying detector refuses any
// text carrying an insurance signal, so a travel-insurance guide about a
// hurricane keeps its own three pins.
export function isDisasterArticle(article) {
  const category = String(article?.category || "");
  if (category === DISASTER_CATEGORY_LABEL) return true;
  const axis = String(article?.contentAxis?.id || article?.axis || "");
  if (axis.startsWith("disaster-")) return true;
  return Boolean(detectDisasterAxis(`${article?.title || ""} ${article?.keyword || ""}`));
}

export function isWellnessArticle(article) {
  const category = String(article?.category || "");
  if (category === "Travel Wellness & Everyday Fitness") return true;
  const axis = String(article?.contentAxis?.id || article?.axis || "");
  if (axis.startsWith("wellness-")) return true;
  return Boolean(detectWellnessAxis(`${article?.title || ""} ${article?.keyword || ""}`));
}

// The four wellness pins. Same rule as the travel set - each one is cut from a
// DIFFERENT part of the article - but the sections are the ones the wellness
// MASTER skeleton actually writes.
function wellnessSources(article, body) {
  const situation = sectionItems(body, /miji|situation|why this happens/i);
  const routine = sectionItems(body, /the routine|routine|moves|exercises/i);
  const habit = sectionItems(body, /building the habit|habit|daily/i);
  const cautions = sectionItems(body, /before you start|when to stop|caution/i);
  const notes = [];
  if (!routine.length) notes.push("본문에 ‘The Routine’ 절이 없어 3번(체크리스트형) 문구는 요약·메타 설명에서 가져왔습니다.");
  if (!situation.length) notes.push("본문에 ‘Miji’s Situation / Why This Happens’ 절이 없어 2번(문제·해결형) 문구는 핵심 질문에서 가져왔습니다.");

  const tpl = Object.fromEntries(WELLNESS_PIN_TEMPLATES.map((t) => [t.id, t]));
  const sources = [
    {
      id: "question",
    ...
    {
      id: "problem-solution",
      overlay: overlayFrom(situation[0] || article?.coreQuestion || article?.title),
      title: titleFrom(tpl["problem-solution"].titlePrefix, situation[0] || article?.coreQuestion || article?.title),
      description: composeDescription(
        [...situation, article?.coreQuestion, article?.metaDescription, ...routine],
        WELLNESS_CTA["problem-solution"],
      ),
    },
    {
      id: "checklist",
      overlay: overlayFrom(routine[0] || article?.desiredReaderAction || article?.title),
      title: titleFrom(tpl.checklist.titlePrefix, routine[0] || article?.desiredReaderAction || article?.title),
      description: composeDescription(
        [...routine, article?.desiredReaderAction, article?.quickAnswer, article?.metaDescription],
        WELLNESS_CTA.checklist,
      ),
    },
    {
      id: "miji-lifestyle",
      overlay: overlayFrom(habit[0] || situation[0] || article?.title),
      title: titleFrom(tpl["miji-lifestyle"].titlePrefix, habit[0] || situation[0] || article?.title),
      // The cautions are deliberately in this pool: the lifestyle pin is the one
      // most likely to read as a promise, so the article's own "what it should
      // not feel like" sentence travels with it.
      description: composeDescription(
        [...habit, ...situation, ...cautions, article?.metaDescription],
        WELLNESS_CTA["miji-lifestyle"],
      ),
    },
  ];
...
// The five preparedness pins. Every one is cut from the article's own
// preparedness half — the kit, the checklist, the plan — and never from the
// condolence or situation sections, so a casualty sentence can never end up as
// pin copy.
function disasterSources(article, body) {
  const household = sectionItems(body, /prepare your own|your own household|household preparedness/i);
  const kit = sectionItems(body, /emergency kit|what to put in|supplies|checklist/i);
  const plan = sectionItems(body, /plan|evacuat|communication|meeting point/i);
  const steps = sectionItems(body, /before|during|after|what to do/i);
  const notes = [];
  if (!household.length) notes.push("본문에 ‘Prepare Your Own Household’ 절이 없어 대비 관련 핀 문구는 요약·메타 설명에서 가져왔습니다.");

  const tpl = Object.fromEntries(DISASTER_PIN_TEMPLATES.map((t) => [t.id, t]));
  // A before/during/after list writes its items as "Before: charge everything…".
  // leadClause() cuts at the colon, which would put the single word "Before" on
  // the pin, so the phase label is dropped before the clause is taken.
  const unlabel = (s) => String(s || "").replace(/^\s*(?:before|during|after)\s*[:—-]\s*/i, "");
  const pick = (...pools) =>
    unlabel(pools.find((p) => p && p.length)?.[0] || article?.desiredReaderAction || article?.metaDescription || article?.title);
  const source = (id, seed, pool) => ({
    id,
    overlay: overlayFrom(seed),
    title: titleFrom(tpl[id].titlePrefix, seed),
    description: composeDescription(pool, DISASTER_CTA[id]),
  });

  const sources = [
    source("emergency-kit-checklist", pick(kit, household), [...kit, ...household, article?.quickAnswer, article?.metaDescription]),
    source("flood-preparedness-checklist", pick(steps, household), [...steps, ...household, article?.metaDescription]),
    source("go-bag", pick(kit, plan, household), [...kit, ...plan, article?.desiredReaderAction, article?.metaDescription]),
    source("family-evacuation", pick(plan, household), [...plan, ...household, article?.metaDescription]),
    source("before-during-after", pick(steps, household, kit), [...steps, article?.quickAnswer, article?.metaDescription]),
  ];
  return { sources, notes };
}

// The pins. Each one is built from a DIFFERENT part of the article, which is
// what makes the copy genuinely different rather than a reworded clone. Three
// for the travel-insurance cluster, four for Travel Wellness, five for Disaster
// Preparedness.
export function buildVariants({ article, storedKit = null, now = new Date() } = {}) {
  const body = article?.bodyHtml || article?.bodyMarkdown || "";
  const disaster = isDisasterArticle(article);
  const wellness = !disaster && isWellnessArticle(article);
  const meta = disaster ? DISASTER_VARIANTS : wellness ? WELLNESS_VARIANTS : VARIANTS;
  const offsets = disaster ? DISASTER_PIN_OFFSET_DAYS : wellness ? WELLNESS_PIN_OFFSET_DAYS : VARIANT_OFFSET_DAYS;
  const link = String(article?.publishedUrl || "").trim();
  const brandLine = brandLineFor(link);
  const assets = pickAssets(
    article,
    disaster ? DISASTER_ASSET_ORDER : wellness ? WELLNESS_ASSET_ORDER : TRAVEL_ASSET_ORDER,
  );
  const hashtags = hashtagsFor(article);

  if (disaster) {
    const { sources, notes } = disasterSources(article, body);
    return {
      variants: assembleVariants({ sources, meta, offsets, assets, article, brandLine, hashtags, link, storedKit: null, now }),
      notes,
      boards: suggestDisasterPinBoards(article),
    };
  }

  if (wellness) {
    const { sources, notes } = wellnessSources(article, body);
    return {
      variants: assembleVariants({ sources, meta, offsets, assets, article, brandLine, hashtags, link, storedKit, now }),
      notes,
      boards: suggestPinBoards(article),
    };
  }

  return buildTravelVariants({ article, body, meta, offsets, assets, brandLine, hashtags, link, storedKit, now });
}

// Shared assembly: turns copy sources into the variant records the screen
// renders. Identical for both categories - only the copy and the image order
// differ above.
function assembleVariants({ sources, meta, offsets, assets, article, brandLine, hashtags, link, storedKit, now }) {
  return sources.map((s, i) => {
    ...
    const image = useStored
      ...
      ? buildPinImage({
          ...
          hook: s.overlay,
          brandLine,
          filename: i === 0 ? pinFilename(article.id) : `${pinFilename(article.id)}-${info.id}`,
        })
      : null;
    return {
      id: info.id,
      ...
      suggestedDate: addDays(now, offsets[i] ?? offsets[offsets.length - 1]),
      reusedStoredKit: Boolean(useStored),
    };
  });
}

function buildTravelVariants({ article, body, meta, offsets, assets, brandLine, hashtags, link, storedKit, now }) {
...
// 게시 상태 필드가 없던 초기 기록도 그대로 읽을 수 있게 채워 준다. 원본을
// 고치지 않고 새 객체를 돌려주므로, 저장하는 쪽은 반드시 아래 upsertRecord로
// 목록에 다시 넣어야 한다 — 사본만 고치면 파일에는 아무것도 남지 않는다.
export function withPostingDefaults(record) {
  if (!record) return null;
  return { ...record, posted: { ...emptyPosted(), ...(record.posted || {}) }, postLog: record.postLog || [] };
}

// 같은 글의 기록은 하나뿐이다: 있으면 교체하고, 없을 때만 추가한다.
export function upsertRecord(records = [], record) {
  const at = records.findIndex((r) => r.articleId === record.articleId);
  const next = [...records];
