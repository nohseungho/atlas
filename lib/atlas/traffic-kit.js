...
// for the travel-insurance cluster, four for Travel Wellness.
export function buildVariants({ article, storedKit = null, now = new Date() } = {}) {
  const body = article?.bodyHtml || article?.bodyMarkdown || "";
  const wellness = isWellnessArticle(article);
  const meta = wellness ? WELLNESS_VARIANTS : VARIANTS;
  const offsets = wellness ? WELLNESS_PIN_OFFSET_DAYS : VARIANT_OFFSET_DAYS;
  const link = String(article?.publishedUrl || "").trim();
  const brandLine = brandLineFor(link);
  const assets = pickAssets(article, wellness ? WELLNESS_ASSET_ORDER : TRAVEL_ASSET_ORDER);
