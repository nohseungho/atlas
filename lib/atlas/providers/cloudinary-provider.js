...
// Uploads a freshly GENERATED image (base64, data URI, or a remote https URL —
// e.g. OpenAI output) and returns a delivery URL transformed to a guaranteed
// 1600x900 WebP. Fixed public_id (=role key) + folder (=slug) keeps re-runs
// idempotent: the same role always overwrites the same asset, never duplicates.
export async function uploadArticleImageData({ slug, key, source }) {
  if (!isCloudinaryConfigured()) {
    const err = new Error("Cloudinary is not configured");
    err.code = "CLOUDINARY_CONFIG_MISSING";
    throw err;
  ...
  const uploadSrc = raw.startsWith("http") || raw.startsWith("data:") ? raw : `data:image/png;base64,${raw}`;
  const result = await cloudinary.uploader.upload(uploadSrc, {
    resource_type: "image",
    folder: `atlas/articles/${slug}`,
    public_id: key,
    overwrite: true,
  ...
  return { secureUrl: deliveryUrl, width: 1600, height: 900 };
}
