...
// Operating policy "paid_api_forbidden": a paid generation call is only ever
// made when the operator has explicitly opted in with ATLAS_ALLOW_PAID_API=1.
// Having a key alone is not consent — images normally arrive as ChatGPT
// hand-off files and go through Cloudinary.
export function paidApiAllowed(environment = process.env) {
  return String(environment?.ATLAS_ALLOW_PAID_API || "") === "1";
}
...
export function imageProviderReadiness() {
  ...
  if (!paidApiAllowed()) {
    return {
      ...
      errorCode: "PAID_API_FORBIDDEN",
      envNeeded: ["ATLAS_ALLOW_PAID_API"],
      message: "운영 정책상 유료 이미지 생성 API는 ATLAS_ALLOW_PAID_API=1 로 명시한 환경에서만 사용합니다.",
    };
  }
  return {
    ready: key,
...
function masterFileFor(asset) {
  const relative = String(asset?.masterAssetPath || "").trim();
  if (!relative) return "";
  const characterDir = path.join(process.cwd(), "public", "atlas", "characters");
  const absolute = path.join(characterDir, path.basename(relative));
  return fs.existsSync(absolute) ? absolute : "";
}

async function generateOne(asset, apiKey, model) {
  const prompt = String(asset?.prompt || "").trim();
  const masterFile = asset?.characterRequired ? masterFileFor(asset) : "";
  if (asset?.characterRequired && !masterFile) {
    return { ok: false, retryable: false, reason: `character master missing: ${asset.masterAssetPath || "unknown"}` };
  }
  ...
  if (masterFile) {
    ...
    form.append("image", new Blob([fs.readFileSync(masterFile)], { type: "image/png" }), path.basename(masterFile));
...
export async function generateImages(visualAssets = []) {
  ...
  if (!r.ready) {
    return { ok: false, blocked: "BLOCKED_IMAGE_PROVIDER", policyCode: r.errorCode || null, envNeeded: r.envNeeded, message: r.message, generated: [] };
  }
  ...
  for (const asset of visualAssets) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        last = await generateOne(asset, apiKey, model);
