...
export const bloggerProvider = {
  ...
  // 기존 게시글의 본문만 갱신한다 (posts.patch). title/labels/publish date는
  // 요청 바디에 아예 포함하지 않아, Blogger가 기존 값을 그대로 유지한다.
  // content: { html }
  ...
  async listDrafts(bloggerBlogId, accessToken) {
    const url = `${BLOGGER_API_BASE}/blogs/${bloggerBlogId}/posts?status=DRAFT&view=ADMIN&fetchBodies=false&fetchImages=false&maxResults=100`;
    const res = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok) {
      if (res.status === 401) {
        const err = new Error("Unauthorized when listing drafts");
        err.code = "TOKEN_EXPIRED";
        throw err;
      }
      const errData = await res.json().catch(() => ({}));
