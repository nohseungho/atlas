...
export const bloggerProvider = {
  ...
  // 공개 글을 초안으로 되돌린다(posts.revert). 글·본문·이미지는 지우지 않고 공개만 내린다.
  // 다시 공개하려면 사용자 승인 후 Blogger에서 publish 해야 한다. 삭제(posts.delete)는 쓰지 않는다.
  async revertPost(bloggerBlogId, postId, accessToken) {
    const res = await fetch(`${BLOGGER_API_BASE}/blogs/${bloggerBlogId}/posts/${postId}/revert`, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (!res.ok) {
      const errData = await res.json().catch(() => ({}));
      if (res.status === 401) {
        const err = new Error(errData.error?.message || "Unauthorized when reverting post");
        err.code = "TOKEN_EXPIRED";
        throw err;
      }
      const err = new Error(errData.error?.message || `Blogger API error ${res.status}`);
    ...
    const post = await res.json();
    return { id: post.id, status: post.status || "", url: post.url || "" };
  },

  // ─── Pages (정책 페이지) ───────────────────────────────────────────────────
  // pages.list / pages.get / pages.patch. 페이지는 발행 파이프라인과 무관한 정적
  // 정책 문서(Privacy, About 등)라 patch(부분 갱신)만 제공한다. 생성·삭제 없음.
  async listPages(bloggerBlogId, accessToken) {
    const res = await fetch(`${BLOGGER_API_BASE}/blogs/${bloggerBlogId}/pages?view=ADMIN&fetchBodies=false`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (!res.ok) {
      if (res.status === 401) {
        const err = new Error("Unauthorized when listing pages");
        err.code = "TOKEN_EXPIRED";
        throw err;
      }
      const errData = await res.json().catch(() => ({}));
    ...
    const data = await res.json();
    return (data.items || []).map((p) => ({ id: p.id, title: p.title || "", url: p.url || "", status: p.status || "" }));
  },

  async getPage(bloggerBlogId, pageId, accessToken) {
    const res = await fetch(`${BLOGGER_API_BASE}/blogs/${bloggerBlogId}/pages/${pageId}?view=ADMIN`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (!res.ok) {
      if (res.status === 401) {
        const err = new Error("Unauthorized when fetching page");
        err.code = "TOKEN_EXPIRED";
        throw err;
      }
      if (res.status === 404) return null;
    ...
    return { id: page.id, title: page.title || "", url: page.url || "", status: page.status || "", content: page.content || "" };
  },

  // content: { html } — 본문만 patch. 제목·URL·상태는 건드리지 않는다.
  async updatePage(bloggerBlogId, pageId, content, auth) {
    if (content?.html === undefined) throw new Error("updatePage: html이 필요합니다.");
    const res = await fetch(`${BLOGGER_API_BASE}/blogs/${bloggerBlogId}/pages/${pageId}`, {
      method: "PATCH",
