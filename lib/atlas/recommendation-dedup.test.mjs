...
// R2 추천 카드에서 생긴 job(rec_*)은 이 고정 시나리오 시점에 없었다. 사용자가
// 새 주제로 글을 시작할 때마다 rec_* job이 하나씩 늘어 후보 수가 줄어드는데,
// 그때마다 이 과거 상태 재현이 깨지지 않도록 함께 제외한다.
const jobsWithoutKw023 = jobs.filter(
  (j) => j.moneyHunterId !== "kw_023" && !String(j.moneyHunterId || "").startsWith("rec_"),
);
// 이 고정 시나리오 시점의 글 목록은 art_013까지다. 이후에 쓴 글(art_014, art_015…)은
// 그때 없었으므로 함께 제외한다. 새 글이 늘 때마다 후보 수가 줄어 이 과거 상태
// 재현이 깨지던 것을 막는다.
const articlesBeforeKw023 = articles.filter((a) => String(a.id) <= "art_013");
