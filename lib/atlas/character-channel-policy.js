...
export const ATLAS_CHARACTERS = Object.freeze({
  ...
  // 확정 수호는 멀티탭 글(logNo 224412747695)에 게시된 따뜻한 애니메이션 일러스트다.
  // ATLAS-SUO-MASTER.png는 그 원본(featured.png)을 잘라낸 얼굴 기준 파일이며, 실사 인물 사진은
  // public/atlas/characters/archive/ 로 옮겨 국내 렌더에 쓰지 않는다(2026-09-20 교정).
  suho: Object.freeze({
    id: "suho",
    displayName: "수호",
    masterFileName: "ATLAS-SUO-MASTER.png",
    masterAssetPath: "public/atlas/characters/ATLAS-SUO-MASTER.png",
    masterSourceAssetPath: "public/atlas/korea/suho/multitap/featured.png",
    visualMode: "warm-practical-illustration",
    styleReferenceAssetPath: "public/atlas/korea/suho/multitap/featured.png",
    identityPrompt:
      "Suho, the same young Korean man from the warm anime-style illustration reference: tousled dark-brown hair, large dark eyes, bright friendly smile, dark green knit sweater; keep the illustration style, never a photographic or older-looking man",
  }),
});
...
export function validateChannelIdentity(record = {}, channelId) {
  ...
  if (record.assetNamespace && record.assetNamespace !== channel.assetNamespace) issues.push(`assetNamespace must be ${channel.assetNamespace}`);
  ...
  // The other channel's presenter must not be named as the person in an image either
  // (a global post once shipped "Suho checks a trail map..." as its featured alt).
  const foreignPresenter = channel.id === ATLAS_CHANNEL_ID.GLOBAL_BLOGGER ? /\bsuho\b|수호/i : /\bmiji\b/i;
  for (const asset of record.images || record.visualAssets || []) {
    if (asset?.characterId && asset.characterId !== character.id) issues.push(`image characterId must be ${character.id}`);
    if (asset?.channelId && asset.channelId !== channel.id) issues.push(`image channelId must be ${channel.id}`);
    if (foreignPresenter.test(`${clean(asset?.alt)} ${clean(asset?.prompt)}`)) issues.push(`image "${asset?.key || asset?.id || "asset"}" names the other channel's presenter; expected ${character.id}`);
  }
  if (sources.some((src) => Object.values(ATLAS_CHARACTERS).some((other) => other.id !== character.id && src.includes(other.masterFileName)))) {
    issues.push(`another channel's character master detected; expected ${character.masterFileName}`);
  }
