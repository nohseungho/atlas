...
import { ATLAS_CHANNEL_ID, bindVisualAssetsToChannel, stampChannelIdentity } from "@/lib/atlas/character-channel-policy";
...
function stepValidating(job) {
  ...
  const record = stampChannelIdentity({
    ...draft,
    id,
    status: "written",
    publishedUrl: "",
    blogId: "",
    visualAssets: bindVisualAssetsToChannel(draft.visualAssets, ATLAS_CHANNEL_ID.GLOBAL_BLOGGER),
    createdAt: now,
    updatedAt: now,
...
async function stepGenerateImages(job) {
  ...
  const stamped = stampChannelIdentity(article, ATLAS_CHANNEL_ID.GLOBAL_BLOGGER);
  Object.assign(article, stamped);
  article.visualAssets = bindVisualAssetsToChannel(article.visualAssets, ATLAS_CHANNEL_ID.GLOBAL_BLOGGER);
  ...
  if (gen.blocked) return { blocked: gen.blocked, policyCode: gen.policyCode || null, message: gen.message || "이미지를 생성할 provider가 연결되지 않았습니다.", envNeeded: gen.envNeeded };
