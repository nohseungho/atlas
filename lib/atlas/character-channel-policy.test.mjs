...
test("Korea Naver always stamps Suho and who-ami", () => {
  ...
  assert.equal(draft.visualMode, "warm-practical-illustration");
  assert.match(draft.styleReferenceAssetPath, /public\/atlas\/korea\/suho\//);
...
test("a global visual whose alt or prompt names Suho is rejected; Korea rejects Miji-named images", () => {
  const global = validateChannelIdentity({ channelId: "global_blogger", character: "miji", visualAssets: [{ key: "featured", alt: "Suho checks a paper trail map", localSrc: "/images/articles/x/featured.png" }] }, ATLAS_CHANNEL_ID.GLOBAL_BLOGGER);
  assert.equal(global.ok, false);
  assert.ok(global.issues.some((i) => /names the other channel's presenter/.test(i)));
  const fine = validateChannelIdentity({ channelId: "global_blogger", character: "miji", visualAssets: [{ key: "featured", alt: "Miji reviews a trail map", localSrc: "/images/articles/x/featured.png" }] }, ATLAS_CHANNEL_ID.GLOBAL_BLOGGER);
  assert.equal(fine.ok, true);
  const korea = validateChannelIdentity({ channelId: "korea_naver", character: "suho", images: [{ id: "i1", alt: "Miji holding a kettle", src: "" }] }, ATLAS_CHANNEL_ID.KOREA_NAVER);
  assert.equal(korea.ok, false);
});
