...
import { insertFiguresIntoLiveHtml, insertIntoPageHtml, replaceLiveImage } from "./live-refresh.js";
...
test("replaceLiveImage swaps exactly one img src/alt and leaves every other byte untouched", () => {
  const live = '<p>intro</p><img src="https://res.cloudinary.com/x/v1/atlas/articles/slug/featured?_a=1" alt="Suho checks a map" loading="eager"><h2>FAQ</h2><img src="https://res.cloudinary.com/x/v1/atlas/articles/slug/context" alt="gear"><p><a href="https://example.org/s">Sources</a></p>';
  const r = replaceLiveImage(live, { matchSrc: "/atlas/articles/slug/featured", newSrc: "https://res.cloudinary.com/x/v2/atlas/articles/slug/featured-miji.png", newAlt: "Miji reviews a trail map" });
  assert.equal(r.applied, true);
  assert.equal(r.before.alt, "Suho checks a map");
  assert.match(r.html, /featured-miji\.png" alt="Miji reviews a trail map" loading="eager"/);
  assert.equal((r.html.match(/<img\b/g) || []).length, 2);
  assert.equal(r.html.replace(/<img[^>]*featured-miji[^>]*>/, "IMG"), live.replace(/<img[^>]*featured\?_a=1[^>]*>/, "IMG"));
  // ambiguous / missing / non-https targets are refused
  assert.equal(replaceLiveImage(live, { matchSrc: "/atlas/articles/slug/", newSrc: "https://res.cloudinary.com/x/y.png", newAlt: "x" }).applied, false);
  assert.equal(replaceLiveImage(live, { matchSrc: "/nope", newSrc: "https://res.cloudinary.com/x/y.png", newAlt: "x" }).applied, false);
  assert.equal(replaceLiveImage(live, { matchSrc: "/featured", newSrc: "http://insecure/y.png", newAlt: "x" }).applied, false);
});
